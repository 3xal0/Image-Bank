...
class PageupController extends BDD{
...
function Selector($id,$page){
    session_start();
    $i=0;
    $tab = array();
        $img = $this->getTags('stockage',end($_SESSION['offset'])); //get images and they parameters (id's,tag id's,url )
        $length = count($img);
        for ($n = 0; $n < $length; $n++) {
            $val = $img[$n];
            $cursor=$val[0];
...
return $tab;
    ...
    function Research($data, $page)
    {
        ...
        $select = $this->Selector($parse,$page);
        return $select;
    }
